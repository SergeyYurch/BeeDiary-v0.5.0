...
describe('SaUsersController (e2e)', () => {
  ...
  let accountsTestHelpers: AccountsTestHelpers;
  const users = [];
  let inputUser1: UserInputModel;

  beforeAll(async () => {
    app = await getApp();
    usersTestService = new UsersTestHelpers(app);
    testingTestHelpers = new TestingTestHelpers(app);
    accountsTestHelpers = new AccountsTestHelpers(app);
    inputUser1 = accountsTestHelpers.getUserInputModel(1);
  });
  ...
  it('POST: [HOST]/sa/users (POST) Add new user to the system. Should return 201 and add new user to db', async () => {
    ...
    await accountsTestHelpers.createSetOfUsers(11, 2);
  });
